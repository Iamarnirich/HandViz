...
function whichSideAttacks(action, possession, team) {
  ...
  const m = (possession || "").toLowerCase().match(/^possession\s+(.+?)\s*_\s*(.+?)\s*_/i);
  if (m && team) {
    const left = (m[1] || "").trim(), right = (m[2] || "").trim();
    const opp = left === team ? right : left;
    if (a.startsWith("attaque " + opp) || a.startsWith("ca " + opp) || a.startsWith("er " + opp) || a.startsWith("mb " + opp) || a.startsWith("transition " + opp)) return "opp";
    if (a.startsWith("attaque " + team) || a.startsWith("ca " + team) || a.startsWith("er " + team) || a.startsWith("mb " + team) || a.startsWith("transition " + team)) return "us";
  }
  const m2 = a.match(/^(attaque|ca|er|mb|transition)\s+([^\(]+)/);
