...
export default function PlayerReportsPanel({ events, jeLinks, match, joueur }) {
  const kpi = useMemo(() => {
    ...
    // On colle à ta requête SQL : on utilise UNIQUEMENT l’équipe de la joueuse
    ...
    if (!team) {
      return {
        ...
        defensif: { pertesBalleAdv: 0, actionsPos: 0, actionsNeg: 0, ratios: { placee: 0, ge: 0, global: 0 } },
      };
    }

    // 1) Lier joueuse → événements (id_joueuse ↔ id_evenement)
    ...
    // Les events passés au composant sont déjà filtrés (match / équipe) par le layout.
    // On croise encore avec la table pivot pour n'avoir que ceux de la joueuse.
    const playerEvents = (events || []).filter((e) => evtIdSet.has(e.id));

    // 2) Passes décisives : on lit seulement evenements.passe_decisive
    //    (et on reste dans le même scope de données que le composant : events filtrés)
    ...
    // 3) Efficacités + 4) Actions +/− (style startsWith ultra simple)
    let attemptsAll = 0,
      goalsAll = 0,
      attempts7m = 0,
      goals7m = 0;
    ...
    playerEvents.forEach((e) => {
      const r = norm(e?.resultat_cthb);   // ex: "but aix - ..." / "tir hc aix"
      const a = norm(e?.nom_action);      // ex: "attaque aix ..." / "att 7m aix ..."
      const s = norm(e?.secteur);         // ex: "7m"
      const z = norm(e?.sanctions);       // ex: "2min prov ..."

      // ---- Détections avec ton style startsWith(`xxx ${team}`)
      const isGoal       = r.startsWith(`but ${team}`);
      const isShot       = isGoal || r.startsWith(`tir ${team}`) || r.startsWith(`tir hc ${team}`);
      const isSeven      = s.includes("7m") || a.startsWith(`att 7m ${team}`);
      ...
      if (z.includes("2min prov") || z.includes("2 min prov") || z.includes("2mn prov") || z.includes("2 mn prov"))
        pos_2min_prov += 1;
    ...
    const actionsPosTotal = pos_buts + pos_7mobtenus + passesDec + pos_2min_prov + pos_plus;
    const actionsNegTotal = neg_pertes + neg_tirHC + neg_arrets + neg_minus;

    // Ratios affichés au-dessus des jauges = bien buts/tirs et buts7m/tirs7m
    ...
    return {
      ...
      // Partie défensive laissée simple
      defensif: { pertesBalleAdv: 0, actionsPos: 0, actionsNeg: 0, ratios: { placee: 0, ge: 0, global: 0 } },
    };
      ...
      <div className="bg-[#b3974e] text-white rounded-[28px] p-6">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          <Card title="Passes décisives">
            <div className="text-2xl font-bold">{kpi?.offensif.passesDec || 0}</div>
          </Card>
          <Card title="Actions Positives">
            <div className="text-2xl font-bold">{kpi?.offensif.actionsPos.total || 0}</div>
            <div className="text-xs mt-1 text-gray-700">AP {kpi?.offensif.actionsPos.ap || 0}</div>
            <div className="text-xs text-gray-700">GE 0</div>
          </Card>
          <Card title="Actions Négatives">
            <div className="text-2xl font-bold">{kpi?.offensif.actionsNeg.total || 0}</div>
            <div className="text-xs mt-1 text-gray-700">AP {kpi?.offensif.actionsNeg.ap || 0}</div>
            <div className="text-xs text-gray-700">GE 0</div>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <Card title="Ratio Offensif Attaque Placée">
            <div className="text-xl font-bold">{(kpi?.offensif.ratios.ap || 0).toFixed(1)}</div>
          </Card>
          <Card title="Ratio Offensif Grand Espace">
            <div className="text-xl font-bold">{(kpi?.offensif.ratios.ge || 0).toFixed(1)}</div>
          </Card>
          <Card title="Ratio Offensif Global">
            <div className="text-xl font-bold">{(kpi?.offensif.ratios.global || 0).toFixed(1)}</div>
          </Card>
        </div>
      </div>
      ...
      <div className="bg-[#3B3B3B] text-white rounded-[28px] p-6">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          <Card title="Pertes de balle">
            <div className="text-2xl font-bold">{kpi?.defensif.pertesBalleAdv || 0}</div>
          </Card>
          <Card title="Actions Positives">
            <div className="text-2xl font-bold">{kpi?.defensif.actionsPos || 0}</div>
          </Card>
          <Card title="Actions Négatives">
            <div className="text-2xl font-bold">{kpi?.defensif.actionsNeg || 0}</div>
          </Card>
        </div>

        <div className="grid grid-cols-3 gap-6">
          <Card title="Ratio Défensif Défense Placée">
            <div className="text-xl font-bold">{(kpi?.defensif.ratios.placee || 0).toFixed(1)}</div>
          </Card>
          <Card title="Ratio Défensif Grand Espace">
            <div className="text-xl font-bold">{(kpi?.defensif.ratios.ge || 0).toFixed(1)}</div>
          </Card>
          <Card title="Ratio Défensif Global">
            <div className="text-xl font-bold">{(kpi?.defensif.ratios.global || 0).toFixed(1)}</div>
          </Card>
        </div>
      </div>
    </div>
